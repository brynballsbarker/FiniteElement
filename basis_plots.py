...
results = pd.read_csv('toplot.csv')
...
results = pd.read_csv('toplotbern.csv')
...
if to_plot:
    for f_ord, f_lab in zip(f_ords,f_labs):
        ...
        # Display the plot
        plt.suptitle(f_lab)
        plt.savefig('plots/solutions_f_order_{}'.format(f_ord)+'.png')
        plt.show()
...
plt.savefig('plots/log_log_errors.png')
plt.show()
"""
